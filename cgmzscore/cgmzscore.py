...
class Observation(object):
    def __init__(self, chart, weight, muac,
                age_in_months, sex, height, logger_name):
    ...
    def get_values(self, growth):
        ...
        if self.chart in ["wfh", "wfl"]:
            ...
            raise exceptions.DataError(
                "SCORES NOT FOUND BY MONTHS : %s", self.age_in_months)
...
class Calculator(object):
    ...
    def __init__(self, logger_name='z_score_log.log', log_level='INFO'):
    ...
    def zScore(self, weight=None, muac=None, age_in_months=None, sex=None, height=None):
        wfa = self.zScore_wfa(
            weight=weight, age_in_months=age_in_months, sex=sex)
        dummy = ''
        if(D(age_in_months) > 24):
            dummy = 'Z_score_WFH'
            wfl = self.zScore_wfh(
                weight=weight, age_in_months=age_in_months, sex=sex, height=height)
        else:
            dummy = 'Z_score_WFL'
            wfl = self.zScore_wfl(
                weight=weight, age_in_months=age_in_months, sex=sex, height=height)
        lhfa = self.zScore_lhfa(
            age_in_months=age_in_months, sex=sex, height=height)
        zscore = json.dumps(
    ...
    def z_score_measurement(self, chart, weight, muac, age_in_months, sex, height):
        ...
        obs = Observation(chart, weight, muac, age_in_months,
                          sex, height, self.logger.name)
